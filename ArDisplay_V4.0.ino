...
void PreCharge_Delay(int voltDisplay) {
  lcd.setCursor (0, 3);
  lcd.print("PRE-CHARGING.       ");
  delay(600);
  lcd.setCursor (0, 3);
  lcd.print("PRE-CHARGING..      ");
  delay(600);
  ...
  if (voltDisplay > 90) {
    ...
    while (1) {
      ...
      lcd.setCursor (0, 3);
      lcd.print("PRE-CHARGE FAULT!   ");

      SD_File = SD.open("ESC.log", FILE_WRITE);
      SD_File.println("PRE-CHARGE FAULT!, ");
      SD_File.close();
...
void setup() {
  ...
  PreCharge_Delay (voltDisplay);      //Print wait for pre-charge message
}
...
void loop () {
  ...
  if (isMsg == false) {      //Clear message time-out
    unsigned long currentTime = millis();

    if (currentTime - msgTime >= 2000) {
