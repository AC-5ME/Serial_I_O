...
float AmpMilliSeconds = 0.0;      //Ah counter
...
void Display_Data (uint8_t& packetId, uint8_t& msgByteHigh, uint8_t& msgByteLow, uint16_t& DataVal) {     //Display data
  ...
  switch (packetId) {
    ...
    case 0x02: {      //Print Current
        ...
        float averageAmps = (lastAmpSample + filterCurrentVal) / 2.0f;
        lastAmpSample = filterCurrentVal;
        float newAmpMilliSeconds = averageAmps * (currentMillis - lastSampleMillis);

        AmpMilliSeconds += newAmpMilliSeconds;
        float AmpHours = (AmpMilliSeconds / 3600000);
...
void Warning_Parser(uint16_t& WarningCode)  {        //Warning_code parser
  switch (WarningCode) {
    case 0x0000: {      //No warnings
        ...
        if ((isVoltageWarning == false) && (isCurrentWarning == false) && (isESCTempWarning == false) && (isMotorTempWarning == false)) {
          lcd.setCursor (0, 3);
          lcd.print("                    ");
        } break;
