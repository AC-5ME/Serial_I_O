...
void Display_Data(int erpmRx, int voltDisplay, int ampDisplay, float WhDisplay, int escTempDisplay, int motorTempDisplay, int dutyCycleDisplay) {
  ...
  */
  ...
  double throttlePos = (dutyCycleDisplay);
  //char dutyBuffer[5];
 ...
 // SD_File.print(dutyBuffer);
  ...
  if (faultDisplay > 0) {     //Check for fault code
...
void Fault_Checker() {
  if (isFaults == false) {
    ...
    Serial.print ("faultDisplay: ");
    Serial.println (faultDisplay);
  }
}
...
void loop () {
  ...
  if (time_now > next_request) {
    Get_Data();
    Display_Data (erpmRx, voltDisplay, ampDisplay, WhDisplay, escTempDisplay, motorTempDisplay, dutyCycleDisplay);
    next_request += 1000;
  }
